...
static void MX_SPI1_Init(void)
{
  ...
  /* 80 MHz APB2 / 16 = 5 MHz. SX1280 permits at most 18 MHz; the previous
   * /4 setting produced a 20 MHz SPI clock and was outside specification. */
...
static void MX_TIM2_Init(void)
{
  ...
  htim2.Instance = TIM2;
  htim2.Init.Prescaler = 5;    /* 48 MHz / (5+1) = 8 MHz (HS_TIMER_FREQUENCY) */
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : RADIO_NSS_Pin (PA8) */
  ...
  /*Configure GPIO pin : RADIO_NRESET_Pin (PA0) */
  ...
  /* EXTI interrupt init*/
  HAL_NVIC_SetPriority(COM_TREQ_EXTI_IRQn, 5, 0);
  HAL_NVIC_EnableIRQ(COM_TREQ_EXTI_IRQn);
